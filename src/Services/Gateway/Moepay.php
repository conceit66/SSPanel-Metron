...
use App\Services\Gateway\Epay\Epay_submit_moepay;
use App\Services\Auth;
use App\Services\Config;
use App\Models\Paylist;


class Moepay extends AbstractPayment
{
    ...
    {
        $this->epay['apiurl'] = 'https://jichanpay.cn/submit.php';//
        $this->epay['partner'] = '1063';//易支付商户pid 1174
        $this->epay['key'] = 'jAP9h5j99SqKbj9kJjbG97Gq5QgQa7Dd';//'7crE36UzCPrR5C7656rz3ARXrQeGaUCP';//易支付商户Key
    ...
    public function MetronPay($type, $price, $buyshop, $paylist_id=0)
    {
        ...
        $data = array(
            ...
            "notify_url" => $_ENV['baseUrl'] . "/payment/notify/moepay/".$user->id,
            "return_url" => $_ENV['baseUrl'] . "/user/payment/return",
            "name" => "azure",
            "money" => $pl->total,
        );
        $alipaySubmit = new Epay_submit_moepay($this->epay);
    ...
    public function notify($request, $response, $args)
    {
        ...
        if ($verify_result) {
		    ...
		    $this->postPayment($out_trade_no, 'MoePay');
		    return;
        }else{
            return;
        }
